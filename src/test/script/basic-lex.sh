...
echo "String_escape.deca :"

if test_lex src/test/deca/syntax/invalid/string_escape.deca 2>&1 \
    | grep -q 'token recognition error'
then
    echo "Echec attendu pour test_lex"
else
    echo "Erreur non détectée par test_lex pour string_escape.deca"
fi
