...
public class Or extends AbstractOpBool {
    ...
    protected  void codeGenExpr(DecacCompiler compiler, Destination destination) {
        ...
        compiler.addInstruction(new CMP(new ImmediateInteger(1), destination.getRegister()));
        Label label = compiler.labeller.create("or");
    ...
    protected void codeGenCmp(DecacCompiler compiler, Destination destination, ControlDestination control) {
        if (control.isBranchToElse()) {
            Label label = compiler.labeller.create("or");
            getLeftOperand().codeGenCmp(compiler, destination, new ControlDestination(label).negate());
